...
class PuckCriterion(FailureCriterionBase):
    ...
    def _set_dim(self, value: int):
        if value in [1, 2, 3]:
            ...
            raise AttributeError(f"Dimension of {self.name} cannot be set to {value}. Allowed are 1, 2 or 3.")
    ...
    pma = property(_get_pma, _set_pma,
                         doc="Activates the failure evaluation regarding matrix failure due to tension.")
    pmb = property(_get_pmb, _set_pmb,
                   doc="Activates the failure evaluation regarding matrix failure due to compression.")
    pmc = property(_get_pmc, _set_pmc,
                   doc="Activates the failure evaluation regarding matrix failure due to shear (compression).")
    pd = property(_get_pd, _set_pd,
    ...
    wf_pf = property(_get_wf_pf, _set_wf_pf,
                          doc="Weighting factor of the fiber failure (pf) mode.")
    wf_pma = property(_get_wf_pma, _set_wf_pma,
                     doc="Weighting factor of the matrix failure (pma) mode.")
    wf_pmb = property(_get_wf_pmb, _set_wf_pmb,
                     doc="Weighting factor of the matrix failure (pmb) mode.")
    wf_pmc = property(_get_wf_pmc, _set_wf_pmc,
                     doc="Weighting factor of the matrix failure (pmc) mode.")
    wf_pd = property(_get_wf_pd, _set_wf_pd,
                      doc="Weighting factor of the delamination failure (pd) mode.")
    cfps = property(_get_cfps, _set_cfps,
    ...
    interface_weakening_factor = property(_get_interface_weakening_factor, _set_interface_weakening_factor,
                 doc="Multiplicator for the interlaminar strength if failure mode pd is active. Default is 0.8.")

    force_global_constants = property(_get_force_global_constants, _set_force_global_constants,
                          doc="Whether to use global constants instead of the material-wise properties.")

    p21_neg = property(_get_p21_neg, _set_p21_neg,
                     doc=f"Global inclination factor at \u03C3 2 = 0 for \u03C3 2 < 0")
    p21_pos = property(_get_p21_pos, _set_p21_pos,
                       doc=f"Global inclination factor at \u03C3 2 =0 for \u03C3 2 > 0")
