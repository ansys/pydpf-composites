...
class ResultDefinition:
    ...
    def _set_stress_strain_eval_mode(self, value: str):
        if value in _SUPPORTED_STRESS_STRAIN_EVAL_MODES:
            ...
            raise ValueError(f"Stress strain eval mode '{value} 'is not allowed. Supported are {values}")
    ...
    expression = property(_get_expression, _set_expression,
                      doc="Defines the type of the computation. Supported are {0}"
                          .format(", ".join([v for v in _SUPPORTED_EXPRESSIONS])))
    combined_failure_criterion = property(_get_combined_failure_criterion, _set_combined_failure_criterion,
                          doc="Combined failure criterion with the selected failure criteria, failure modes and "
                              "weighting factors")
    measures = property(_get_measures, _set_measures,
                          doc="Defines the type of failure measurement. Supported are {0}".format(
                              ', '.join([v for v in _SUPPORTED_MEASURES])))
    composite_definitions = property(_get_composite_definitions, _set_composite_definitions,
                                     doc="List of ACP Composite Definitions (.h5) files which contain"
                                         " the lay-up information.")
    assembly_mapping_files = property(_get_assembly_mapping_files, _set_assembly_mapping_files,
    ...
    material_files = property(_get_material_files, _set_material_files, doc="List of material files (.xml or .engd) "
                                                                            "which contain all material properties.")
    write_data_for_full_element_scope = property(_get_write_data_for_full_element_scope,
    ...
    element_scope = property(_get_element_scope, _set_element_scope, doc="List of element which are selected for the "
                                                                         "computation.")
    ply_scope = property(_get_ply_scope, _set_ply_scope, doc="List of plies which are selected for the computation.")
    ...
    max_chunk_size = property(_get_max_chunk_size, _set_max_chunk_size, doc="The computation is chunked. This parameter "
