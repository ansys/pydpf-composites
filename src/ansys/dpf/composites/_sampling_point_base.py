...
class SamplingPointBase:
    ...
    def __init__(
        ...
        if self._server != server:
            load_composites_plugin(self._server)
    ...
    def element_id(self) -> Union[int, None]:
        ...
        """
        raise NotImplementedError("SamplingPoint element_id is not implemented.")
    ...
    def _check_status(self) -> None:
        if not self._isuptodate or not self._results:
            raise RuntimeError(
                f"Results of sampling point {self.name} are not available." " Please update first"
            )
